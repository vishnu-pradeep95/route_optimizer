...
- [x] **GEO-03**: User sees a warning when two or more orders in an upload resolve to GPS coordinates within 15m of each other
- [x] **GEO-04**: Upload results show how many addresses were cache hits (free) vs Google API calls, with estimated cost
...
- [x] **DASH-06**: Sidebar collapses to icon-only on screens below 1280px and uses DaisyUI drawer on mobile
- [x] **DASH-07**: QR sheet prints cleanly with large QR codes, vehicle name, and driver name via @media print

### Driver PWA

- [x] **PWA-01**: Next stop is displayed as a prominent hero card at top of stop list with large address, distance, and Navigate button
...
- [x] **PWA-06**: Route data persists offline -- loading, viewing stops, and marking deliveries work without network
